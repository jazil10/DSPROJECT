...
public class BSTByName {
    ...
    public void Insertbyname(GNode node) 
    {
        ...
        else 
        {
            ...
            {
                N3= N2;
                if (N.node.name.compareTo(N2.node.name)<0)//<N2.node.year) 
                {
                    N2 = N2.left;
                } 
                else 
            ...
            if(N3.node.name.compareTo(N.node.name)<0)//<N.node.year) 
            {
                N3.right=N;
                //System.out.print(node.year+", ");
            }
            else 
            {
                N3.left=N;
    ...
    public String LNRByName(){
         return TraverseByName(root);
    }
    
    public String TraverseByName( BSTNode m){
        if(m==null){
            return null;
        }
        else{
            TraverseByName(m.left);
          
           str = str + "Name: "+ m.node.name + " Year: "+ m.node.year + " Genre: "+ m.node.genre+" Rating: "+ m.node.rating+"\n";
           
           TraverseByName(m.right);
           return str;
        }
        
    }
    
    public BSTNode[] findD(String nm) 
    { 
        if(root ==null){
            ...
            while(curr!=null && curr.node.name.compareTo(nm)!=0){
                parent = curr;
                if(nm.compareTo(curr.node.name)<0){
                    curr = curr.left;
                }
                else{
                    curr= curr.right;
